...
<q-page class="column import-private-key" :class="{'dark-theme': $store.state.settings.lightMode === 'true', 'text-black bg-grey-12': $store.state.settings.lightMode === 'false', 'desktop-marg': screenSize > 1024,  'mobile-pad': screenSize < 1024}">
     <q-header v-if="$q.platform.is.mobile">
                                        ...
                                        <q-stepper :dark="$store.state.settings.lightMode === 'true'" :light="$store.state.settings.lightMode === 'false'" v-model="step" done-color="green" ref="stepper" alternative-labels vertical color="primary" animated flat>
                                            ...
                                            <q-step title="Private Key" :name="1" prefix="1" order="10" :done="step > 1">
                                                <div class="text-black">
                                                    ...
                                                    <span>
                                                        <q-input :dark="$store.state.settings.lightMode === 'true'" :light="$store.state.settings.lightMode === 'false'" debounce="500" rounded outlined color="purple" v-model="addWallet.addressPriv" :error="addWallet.addressError" @input="getPubKeyFromPrivate" @keyup.enter="nextFromPriv" :label="$t('EosAccount.enter_private_key')" />
                                                    </span>
                                                    <div v-show="chainKeyInvalid" class="text-h6 text-red">
                                                        Key invalid
                                                    </div>
                                                    <q-stepper-navigation v-show="chainKeyNext && !chainKeyInvalid" class="flex justify-end">
                                            ...
                                            <!--
                                                    2. Wallet name
                                                    -->
                                            <q-step title="Current Wallet Name" :name="2" prefix="2" order="30" :done="step > 2">
                                            ...
                                            <q-step title="Verto Password" :name="3" prefix="3" order="40" :done="step > 3">
                                                ...
                                                <div class="text-black">
                                                    ...
                                                    <q-stepper-navigation v-show="submitKey" class="flex justify-end">
                                                        <q-btn @click="addChainAddress()" color="deep-purple-14" class="--next-btn" rounded :label="$t('next')" />
                                                    </q-stepper-navigation>
                                                </div>
                                            </q-step>
                                        </q-stepper>
        ...
        <q-card class="mobile-version" :class="$store.state.settings.lightMode === 'true' ? 'text-white mobile-card':'bg-white text-black'">
            <q-header>
                <q-toolbar :class="$store.state.settings.lightMode === 'true' ? 'text-white mobile-card':'bg-white text-black'">
...
export default {
  ...
  methods: {
    ...
    async nextFromPriv () {
      ...
      if (this.addWallet.address) { this.$refs.stepper.next() }
    },
    async getPubKeyFromPrivate () {
      ...
      let pubKeyFromPrivate = {
        ...
        async eth () {
          try {
            let account = await web3.eth.accounts.privateKeyToAccount(self.addWallet.addressPriv)
    ...
    addChainAddress: async function () {
      if (this.submitKey) {
        this.resetErrors()
        const result = await this.$configManager.saveWalletAndKey(this.addWallet.walletName, this.addWallet.vertoPassword, null, this.addWallet.address, this.addWallet.addressPriv, this.currentChain.toLowerCase(), 'import')
