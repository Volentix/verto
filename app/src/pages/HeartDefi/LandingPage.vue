<template>
  <div class="q-px-md">
    <div class="mncright">

      <div class="mncright-left" >

        <div v-if="!$store.state.currentwallet.user"><Intro

          class="full-width connectTo"
        />
          <AssetList />
        </div>
        <div v-else>
         <Loading class=" q-mb-md" v-if="loadingP" />
          <div class="dashboard-area">

            <div class="dashboard-left">
              <div class="dashboard-left_abc1">
                <p>Welcome</p>
                <h2 class="flex welcome flex-center">
                  <img
                    class="avatar"
                    @click="copyToClipboard( $store.state.currentwallet.user.address)"
                    src="data:image/png;base64,"
                  /><span class="cursor-pointer" @click="copyToClipboard( $store.state.currentwallet.user.address)">{{ $store.state.currentwallet.user.addressFriendly }}</span>
                  <q-btn-dropdown flat dropdown-icon="more_vert">
                    <q-list>
                      <q-item @click="copyToClipboard( $store.state.currentwallet.user.address)" clickable v-close-popup>
                        <q-item-section>
                          <q-item-label>Copy address</q-item-label>
                        </q-item-section>
                        <q-item-section side>
                          <q-icon name="content_copy" />
                        </q-item-section>
                      </q-item>
                      <q-item
                        @click="$router.push('/connect/logout')"
                        clickable
                        v-close-popup
                      >
                        <q-item-section>
                          <q-item-label>Disconnect</q-item-label>
                        </q-item-section>
                        <q-item-section side>
                          <q-icon name="logout" />
                        </q-item-section>
                      </q-item>

                      <q-item clickable v-close-popup>
                        <q-item-section>
                          <q-item-label>Rename wallet</q-item-label>
                        </q-item-section>
                        <q-item-section side>
                          <q-icon name="person" />
                        </q-item-section>
                      </q-item>
                      <q-item clickable v-close-popup>
                        <q-item-section>
                          <q-item-label>Show QR Code</q-item-label>
                        </q-item-section>
                        <q-item-section side>
                          <q-icon name="qr_code" />
                        </q-item-section>
                      </q-item>
                    </q-list>
                  </q-btn-dropdown>
                </h2>
              </div>

              <div class="dashboard-left_abc2">
                <div class="dashboard-left_abcc1">
                  <img
                    :src="currentChain.current.icon"
                    alt="images not found"
                  />
                </div>
                <div class="dashboard-left_abcc2">
                  <h5 class="cursor-pointer" @click="switchChain()">
                    {{ currentChain.current.label }}
                    <sup
                      >Switch <img width="15px" :src="currentChain.alt.icon"
                    /></sup>
                  </h5>
                  <h2>
                    ${{ fundTotal }}
                    <sup>
                      <i class="fas fa-sort-up"></i>
                      0.50%
                    </sup>
                  </h2>
                </div>
              </div>

              <div class="dashboard-left_abc3 q-mb-lg">
                <div class="dashboard-left_abbc3">
                  <div class="dashboard-left_abbc3a1">
                    <a href="" @click.prevent="">
                      <div class="dashboard-left_abbc3a2">
                        <img
                          src="data:image/jpeg;base64,"
                        />
                      </div>
                      <p>Borrow</p>
                    </a>
                  </div>
                  <div class="dashboard-left_abbc3a1">
                    <a href="" @click.prevent="">
                      <div class="dashboard-left_abbc3a2">
                        <img
                          :src="require('assets/Heartdefi/img/hex-hex.png')"
                          alt="images not found"
                        />
                      </div>
                      <p>Stake Hex</p>
                    </a>
                  </div>
                  <div class="dashboard-left_abbc3a1">
                    <a href="" @click.prevent="">
                      <div class="dashboard-left_abbc3a2">
                        <img
                          src="data:image/jpeg;base64,"
                          alt="images not found"
                        />
                      </div>
                      <p>Stake PLS</p>
                    </a>
                  </div>
                  <div class="dashboard-left_abbc3a1">
                    <a href="" @click.prevent="">
                      <div class="dashboard-left_abbc3a2">
                        <img
                          src="data:image/png;base64,"
                          alt="images not found"
                        />
                      </div>
                      <p>Trade</p>
                    </a>
                  </div>
                </div>
              </div>
            </div>

            <div class="dashboard-right">
              <ExchangeForm />
            </div>
          </div>
          <AssetsExplorer
            :key="$store.state.currentwallet.user.chain"
            :entity="['assets', 'investments']"
            :filterChains="['eth', 'tpls']"
          />
          <DefiList
            :key="'defi-1' + $store.state.currentwallet.user.address"
            :type="'sacrifice'"
          />
          <AssetList />
          <DefiList
            :key="'defi-2' + $store.state.currentwallet.user.address"
            :type="'track'"
          />
        </div>
      </div>
      <div class="mncright-right">
        <div class="mncright-right2">
          <RightSideTop class="" />
          <NewsArea :mini="true" />

        </div>
      </div>
    </div>
  </div>
</template>

<script>
import Loading from '../../components/Heartdefi/Loading'
import HD from '@/util/hdwallet'
import Intro from './Intro.vue'
import ExchangeForm from '../../components/HeartDefi/ExchangeForm'
import AssetsExplorer from '../../components/Verto/Token/AssetsExplorer'
import AssetList from '../../components/HeartDefi/AssetList'
import DefiList from '../../components/HeartDefi/DefiList'
import RightSideTop from '../../components/HeartDefi/RightSideTop'
// import UpcomingEvents from '../../components/HeartDefi/UpcomingEvents'
import NewsArea from 'components/Heartdefi/NewsArea'
import Formatter from '../../mixins/Formatter'
import Connect from '../../mixins/Connect'
let data = HD.getVertoChains()
export default {
  mixins: [Formatter, Connect],
  components: {
    Intro,
    NewsArea,
    Loading,
    AssetsExplorer,
    ExchangeForm,
    AssetList,
    DefiList,
    RightSideTop
  },
  data () {
    return {
      loadingP: false,
      currentChain: {
        current: {},
        alt: {}
      }
    }
  },
  watch: {
    $route: function () {
      if (this.$route.params.action === 'logout') {
        this.logout()
      }
    },
    '$store.state.currentwallet.user': function (val) {
      this.initUser()
    }
  },
  computed: {},
  created () {
    this.getChainData()
  },
  mounted () {
    this.loadingP = true
    if (this.$store.state.currentwallet.user && this.$store.state.currentwallet.user.address) {
      setTimeout(() => {
        this.loadingP = false
      }, 5000)
    } else {
      this.loadingP = false
    }
  },
  methods: {
    initUser () {
      console.log(123334)
      let val = this.$store.state.currentwallet.user
      if (val && val.address) {
        let init = true
        if (this.$store.state.wallets.tokens && this.$store.state.wallets.tokens.find(o => o.key.toLowerCase() === val.address.toLowerCase())) {
          init = false
        }
        this.loadingP = init
        console.log(init, 'init 2')
        this.getChainData()
        setTimeout(() => {
          this.loadingP = false
        }, 5000)
      }
    },
    switchChain () {
      let temp = this.currentChain.alt
      this.currentChain.alt = this.currentChain.current
      this.currentChain.current = temp
      this.$store.state.currentwallet.user.chain = temp.value
      let v = JSON.parse(
        JSON.stringify(this.$store.state.investment.defaultAccount)
      )
      v.chain = temp.value
      this.$store.commit('investment/setDefaultAccount', v)
    },
    getChainData () {
      this.currentChain.current = data.find(
        (o) => o.value === this.$store.state.currentwallet.user.chain
      )
      this.currentChain.alt = data.find(
        (o) =>
          o.value ===
          (this.$store.state.currentwallet.user.chain === 'eth'
            ? 'tpls'
            : 'eth')
      )
    }
  }
}
</script>
<style scoped>
.connectTo {
  margin-top: 5px;
  margin-bottom: 50px;
}
.avatar {
  border-radius: 50%;
  width: 30px;
  height: 30px;
  margin-right: 10px;
}
.welcome {
  width: fit-content;
}
</style>
