...
export default {
  ...
  methods: {
    ...
    initTable (chain) {
      ...
      this.$store.state.wallets.tokens.filter(o => (!account && !chain) || (chain && o.chain === chain && !account) || (account && o.chain === account.chain && ((account.isEvm && o.key === account.key) || (!account.isEvm && o.name === account.name)))).forEach((token, i) => {
