  ...
  >
    <q-dialog v-model="alertSecurity">
      ...
      >
        ...
        <q-card-section class="text-body1">
          The private key is confidential. Please make sure you do not share it
          with anyone. Your private keys control your funds.
          <br />
          <br />
          ...
          <q-input
            ...
            type="password"
            :label-color="
              $store.state.settings.lightMode === 'true' ? 'white' : ''
            "
    ...
    <div v-if="!($q.platform.is.mobile || $isbex)">
      ...
      >
        <div class="row">
          <div class="col-md-8">
            ...
            <ul class="tabs group">
              <li
                :class="{
                  manage: tokensTotal,
                  read: !tokensTotal && !$route.params.accounts
                }"
              >
                <a
                  @click="
          ...
          <div
            class="col-md-4 row justify-end q-pr-lg"
            v-if="!$route.params.accounts"
          >
            <TokenByAccount
              @filterTokensByAccount="filterTokensByAccount"
              v-if="tab == 'assets' && selectedChain"
              :mode="'select'"
              :chain="selectedChain.chain"
              class="justify-end q-mx-md"
            />
      ...
      >
        <div class="row q-col-gutter-md" v-show="!hideList">
          <AssetGroup
            :key="$store.state.wallets.portfolioTotal+'assets'"
            @setItemAction="setItemAction"
            :getChainTotal="getChainTotal"
            @showAll="showAll"
            :style="groupLayout == 'grid' ? 'background: #3f50b512;' : ''"
            :class="{
              'col-md-6': groupLayout == 'grid',
              'col-md-3': groupLayout == 'list' || !groupLayout
            }"
            v-show="!entity || entity.includes('chains')"
            :card="{
              title: 'Chains',
              view: groupLayout,
              description: 'Portfolio value by chain',
              limit: 6,
              type: 'chains',
              bgColor: 'bg-indigo-1',
              textColor: 'text-indigo-6',
              data: chains
            }"
          />
          <AssetGroup
            ...
            :getChainTotal="getChainTotal"
            @showAll="showAll"
            :style="groupLayout == 'grid' ? 'background: #cbe5e15e;' : ''"
            :class="{
              'col-md-6': groupLayout == 'grid',
              'col-md-3': groupLayout == 'list' || !groupLayout
            }"
            v-show="!entity || entity.includes('assets')"
            :card="{
              title: 'Assets',
              view: groupLayout,
              hideArrow: this.readOnly,
              description: 'Liquid assets',
              limit: 6,
              bgColor: 'bg-teal-1',
              textColor: 'text-teal-6',
              type: 'assets',
              data: assets
            }"
          />
          <AssetGroup
            ...
            :getChainTotal="getChainTotal"
            @showAll="showAll"
            :style="groupLayout == 'grid' ? 'background: #ab47bb21;' : ''"
            :class="{
              'col-md-6': groupLayout == 'grid',
              'col-md-3': groupLayout == 'list' || !groupLayout
            }"
            v-show="
              allInvestments &&
                ...
                (!entity || entity.includes('investments'))
            "
            :card="{
              title: 'Investments',
              view: groupLayout,
              hideArrow: this.readOnly,
              description: 'Staked and locked assets',
              limit: 6,
              bgColor: 'bg-purple-1',
              textColor: 'text-purple-5',
              data: allInvestments,
              type: 'investments'
            }"
          />
          ...
          <AssetGroup
            v-show="!entity || entity.includes('nfts')"
            @setItemAction="setItemAction"
            :getChainTotal="getChainTotal"
            @showAll="showAll"
            :style="groupLayout == 'grid' ? 'background: #00ffff1f;' : ''"
            :class="{
              'col-md-6': groupLayout == 'grid',
              'col-md-3': groupLayout == 'list' || !groupLayout
            }"
            :card="{
              title: 'Farms',
              description: 'Yield generators',
              view: groupLayout,
              hideArrow: this.readOnly,
              limit: 6,
              bgColor: 'bg-cyan-1',
              textColor: 'text-cyan-6',
              type: 'farms',
        ...
        <q-scroll-area
          ...
          :dark="$store.state.settings.lightMode === 'true'"
          :class="{
            receive_wrapper_class_scroll: tab == 'receive',
            import_wrapper_class_scroll: tab == 'import'
          }"
          style="margin-left: -15px; height: 77vh;"
        >
          ...
          >
            <div
              ...
              @click="
                tab !== 'receive' && tab !== 'privateKeys'
                  ...
                  : ''
              "
              v-for="(chain, i) in chains.filter(
                a => tab !== 'privateKeys' || (a.accounts && a.accounts.length)
              )"
              :key="i"
            >
              <div class="main cursor-pointer">
                ...
                >
                  ...
                  <span class="text-caption" v-if="false">3 accounts</span
                  ><br />
               ...
               <!--  <div
                  ...
                  <div
                   ...
                   v-if="
                    ...
                    "
                  >
                    {{ chain.accounts.length }} private key{{chain.accounts.length == 1 ? '' : 's'}}
                    <q-icon name="arrow_forward_ios" />
                  </div>
                  <span
                    class="cursor-pointer"
                ...
                <div
                  ...
                  v-else-if="tab == 'import' || tab == 'create'"
                >
            ...
            <div
              class="col-md-12 flex flex-center text-body1 cursor-pointer"
              v-if="!showAllChains && tab == 'chains'"
              @click="
                showAllChains = true;
                getChains();
              "
      ...
      >
        <q-scroll-area
          :visible="true"
          :dark="$store.state.settings.lightMode === 'true'"
          :class="{ receive_wrapper_class_scroll: tab == 'import' }"
          style="margin-left: -15px; height: 77vh;"
        >
          <div class="sub-top sub-top-chart">
          ...
          >
            <div
              class="col-md-3 asset-card"
              v-show="item.id == 'investments' || listViewMode == 'card'"
              v-for="(asset, i) in filterTokens"
              :key="i"
            >
              <div class="main cursor-pointer">
                <div class="main-top" @click="showTokenPage(asset)">
                  <div class="mt-img">
                    <img
                      :src="asset.icon"
                      style="border-radius:50%"
                      :onerror="defaultToken(asset.chain)"
                    />
                  </div>
                  <div>
                    <h6>
                ...
                >
                  ...
                  <span v-if="asset.notValuable" class="no-value"
                    >No Value</span
                  >

                  <a v-if="!readOnly && false" href="javascript:void(0)">Trade</a>
                </h2>
                ...
                <div class="asset-action q-pt-md row flex">

                  <q-btn label="Send" @click="showTokenPage(asset)" outline :class="{'col-md-4': !canStake(asset),'col-md-3' : canStake(asset) }" /> <q-btn v-if="canStake(asset)" label="Stake" @click="showTokenPage(asset)" outline :class="{'col-md-4': !canStake(asset),'col-md-3' : canStake(asset) }"  class="q-ml-md" />  <q-btn v-if="['eth', 'terra'].includes(asset.chain)" @click="exchangeToken(asset, 'sell')" label="Swap" outline :class="{'col-md-4': !canStake(asset),'col-md-3' : canStake(asset) }"  class="q-ml-md" />

                    <q-icon name="visibility_off" size="sm" class="q-pl-md" @click="markAssetNotValuable(asset)" v-if="!asset.notValuable">
                     <q-tooltip>
                      Set value to 0
                      </q-tooltip>
                      </q-icon>
                    <q-icon name="visibility" size="sm" class="q-pl-md" @click="markAsValuable(asset)" v-else>
                     <q-tooltip>
                      Add value to total balance
                      </q-tooltip>
                    </q-icon>
                 </div>
              </div>
    ...
    /> -->
      ...
      <div class="small-grid" v-if="false">
        <div class="main">
          ...
          <p>Price</p>
          <h2>
            $79 <span class="g-txt">.93</span>
            <span class="sr-txt">↓ 1.3%</span>
            <a href="#">Trade</a>
          </h2>
        </div>
        ...
        <div class="main">
          <div class="main-top">
            ...
            <div>
              <h6>
                Cryptocurrency Index<img src="img/check.png" alt="image" />
              </h6>
            </div>
          </div>
          ...
          <p>Price</p>
          <h2>
            $99 <span class="g-txt">.93</span>
            <span class="sg-txt">↑ 1.3%</span>
            <a href="#">Trade</a>
          </h2>
        </div>
      </div>
    ...
    <div v-if="$q.platform.is.mobile || $isbex">
      <TabAssetsExplorer
        ...
        :getChains="getChains"
        :allChains="allChains"
        :showAllChains.sync="showAllChains"
        :showTokenPage="showTokenPage"
        :initTable="initTable"
        :selectedChain.sync="selectedChain"
        :keys="keys"
        :showPrivateKeys="showPrivateKeys"
        :alertSecurity.sync="alertSecurity"
        :tokenSearchVal="tokenSearchVal"
        :getImportLink="getImportLink"
      />
    </div>
    <!-- MOBILE SECTION END -->
  </div>
...
export default {
    ...
    return {
      ...
      platformOptions: [
        ...
        },
        {
          ...
          value: 'Balancer-labs',
          icon:
            'https://thumbs.dreamstime.com/b/balancer-bal-token-symbol-cryptocurrency-logo-coin-icon-isolated-white-background-vector-illustration-219784418.jpg'
        },
        {
          label: 'yEarn',
          value: 'yEarn',
        ...
        },
        {
          ...
          value: 'Curve',
          icon:
            'https://crypto-monedas.com.mx/wp-content/uploads/2021/04/CRV.png'
        }
      ],
      assetsOptions: [
        {
  ...
  async mounted () {
    ...
    setTimeout(() => {
      this.$store.state.wallets.tokens
        ...
        .forEach(w => {
          if (
            !this.$store.state.currentwallet.userData.ethInvestPools ||
            !this.$store.state.currentwallet.userData.ethInvestPools[w.key.toLowerCase()]
          ) {
            this.getInvestments(w)
          } else {
            this.getInvestedTokens(
              this.$store.state.currentwallet.userData.ethInvestPools[w.key.toLowerCase()],
              88
            )
          }
    ...
    let eosWallets = this.$store.state.wallets.tokens
      .filter(
        o =>
          o.chain === 'eos' &&
          o.type === 'eos' &&
          (!data || !data.find(a => a.owner === o.name.toLowerCase()))
      )
      .map(o => o.name)
    ...
    '$store.state.wallets.tokens': {
      ...
      handler () {
        ...
        if (this.selectedChain) {
          this.setChainWalletData(this.selectedChain)
        }
      }
    },
    allInvestments: {
  ...
  computed: {
    ...
    chainInvestments () {
      let self = this
      let chains = {
        eth: self.assetsOptions[1].data.eth
          .map(v => (isNaN(v.usd) ? 0 : +v.usd))
          .reduce((a, b) => a + b, 0),
        eos: self.assetsOptions[1].data.eos
          .map(v => (isNaN(v.usd) ? 0 : +v.usd))
          .reduce((a, b) => a + b, 0)
      }
      return chains
            ...
            this.$q.platform.is.mobile
              ? (chain === 'eos' &&
                ...
                (chain === 'eth' &&
                  o.owner &&
                  o.owner.toLowerCase() ===
                    this.$store.state.investment.defaultAccount.key.toLowerCase() &&
                  this.$store.state.investment.defaultAccount.chain === 'eth')
              : true
          )
        )
      })
  ...
  methods: {
    canStake (asset) {
      ...
      let can = stakingOptions[asset.chain] && stakingOptions[asset.chain].includes(asset.type)
    ...
    setItemAction (data) {
      if (data.tab === 'chains') {
        ...
        this.hideList = true
        this.setChainWalletData(
          this.chains.find(o => o.chain === data.chain),
          false
        )
    ...
    addPendingHexFetch () {
      ...
      let index = this.assetsOptions[1].data.eth.findIndex(
        o => o.isStaked && o.type === 'hex'
      )
    ...
    async getHexStakes () {
      ...
      let accounts = this.$store.state.currentwallet.config.keys.filter(
        a => a.type === 'eth'
      )
      ...
      accounts.forEach(async f => {
        ...
        let index = this.assetsOptions[1].data.eth.findIndex(
          o => o.isStaked && o.type === 'hex' /* && o.owner === f.key */
        )

        let data = {
          hexStakes: this.$store.state.currentwallet.userData.hexStakes || {}
        }
      ...
      eosChain.accounts
        ...
        .forEach(f => {
          let a = {
            ...
            amount: f.vtxStakes,
            icon:
              'https://www.api.bloks.io/image-proxy/display?w=100&h=100&url=https://raw.githubusercontent.com/BlockABC/eos-tokens/master/tokens/volentixgsys/VTX.png&op=resize'
          }
          let index = this.assetsOptions[1].data.eos.findIndex(
            o => o.isStaked && o.type === 'vtx'
          )
    ...
    setChainWalletData (chain, updateAccount = true) {
      if (chain) {
        ...
        console.log(self.$store.state.wallets.customTotal, ' self.$store.state.wallets.customTotal')
    ...
    getInvestedEosTokens (investments) {
      ...
      const setValue = (t, index) => {
        ...
        let total = tkData
          ? tkData.current_price * parseFloat(t['count' + index])
          : 0
        ...
        return {
          ...
          protocol: 'Defibox',
          protocolIcon:
            'https://defibox.s3.ap-northeast-1.amazonaws.com/eos/token.defi-box.png'
        }
      }
      investments.forEach(item => {
        ...
        indexes.forEach(i => {
          let index = assets.findIndex(
            t =>
              t.type === item['symbol' + i].toLowerCase() &&
              item['contract' + i] === t.contract
          )
      ...
      this.assetsOptions[1].data.eos = this.assetsOptions[1].data.eos
        .filter(o => o.isStaked && o.type === 'vtx')
        .concat(assets)
      this.$set(this, 'assetsOptions', this.assetsOptions)
    },
    ...
    getInvestedTokens (investments, formatted = false) {
        ...
        investments.forEach(t => {
          ...
          t.tokens.forEach(a => {
            let protocolData = this.platformOptions.find(
              o =>
                t.protocolDisplay &&
                o.label.toLowerCase() === t.protocolDisplay.toLowerCase()
            )
            let index = assets.findIndex(
      ...
      if (!formatted) {
        ...
        if (investments[0]) {
          udata.ethInvestPools[investments[0].owner.toLowerCase()] =
            inv && inv[investments[0].owner.toLowerCase()]
              ? inv[investments[0].owner.toLowerCase()].concat(assets)
              : [].concat(assets)
        }
    ...
    getChains () {
      ...
      if (!['create', 'import'].includes(this.tab)) {
        all = this.setChains()
        if (this.selectedChain) {
          this.selectedChain = all.find(m => m.chain === this.selectedChain.chain)
        }
        this.chains = all
          .filter(
            o =>
        ...
        all = HD.getVertoChains()
          .filter(
            c => !this.$store.state.settings.importDisabled.includes(c.value)
          )
          .filter(
            c => !this.$store.state.settings.disableChains.includes(c.value)
          )
        this.chains = all
      } else if (this.tab === 'create') {
        all = HD.getVertoChains().filter(
          c => c.isEvm || ['btc', 'eos'].includes(c.value)
        )
        this.chains = all
      }
      return all
    ...
    initTable (chain = null, account = null) {
      ...
      JSON.parse(
          ...
          this.$store.state.wallets.tokens.filter(
            o =>
              ...
              (account &&
                ...
                ((account.isEvm &&
                  o.key.toLowerCase() === account.key.toLowerCase()) ||
                  (!account.isEvm &&
                    o.name.toLowerCase() === account.name.toLowerCase())))
          )
        )
      ).forEach((asset, i) => {
        ...
        if (
          not_valuable.length &&
          not_valuable.find(
            z => z.chain === token.chain && z.type === token.type
          )
        ) {
          token.tokenPrice = 0
          token.usd = 0
        ...
        if (
          ...
          if (index !== -1) {
            ...
            this.assets[index].usd +=
              isNaN(token.usd) || !token.usd || typeof token.usd === 'undefined'
                ? 0
                : token.usd
      ...
      if (account) {
        this.setChainWalletData(
          this.chains.find(o => o.chain === account.chain),
          false
        )
      }
    },
...
.assets .asset-card:hover .asset-info {
    display:none
}
 .asset-card .asset-action {
    display:none
}
.assets .asset-card:hover .asset-action {
    display:block
}
</style>
