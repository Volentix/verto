                        ...
                        <div v-show="!Array.isArray(accountNames) || !accountNames.length">
                            ...
                            <q-stepper :dark="$store.state.settings.lightMode === 'true'" :light="$store.state.settings.lightMode === 'false'" flat v-model="step" vertical ref="stepper" color="primary" animated>
                                ...
                                <q-step title="Select public key" :name="2" prefix="2" :done="step > 2">
                                    <q-btn flat @click="step = 1" unelevated icon="keyboard_arrow_up" :color="$store.state.settings.lightMode === 'false' ? 'primary' : 'white'" class="--back-btn" />

                                    <div class="text-black">
                                        <div class="text-h4 --subtitle --subtitle__summary">Select the public key for this account</div>
                                        <q-select :dark="$store.state.settings.lightMode === 'true'" :light="$store.state.settings.lightMode === 'false'" separator rounded outlined class="select-input" v-model="currentToken" :options="tokensOption">
                                        ...
                                        <q-input v-model="publicKey" :dark="$store.state.settings.lightMode === 'true'" :light="$store.state.settings.lightMode === 'false'" rounded outlined class="--input" color="deep-purple-14" label="Public key" debounce="500" />
                                    </div>
                                ...
                                <q-step :name="3" title="Send 0.35" icon="money" :done="step > 3">
                                    <q-btn flat @click="step = 2" unelevated icon="keyboard_arrow_up" :color="$store.state.settings.lightMode === 'false' ? 'primary' : 'white'" class="--back-btn" />
                                    <div class="text-black">
                                        <div class="text-h4 --subtitle">
                                            <ul>
                                ...
                                <q-step :name="4" title="Retry Associate with EOS Account" icon="restore" :done="step > 4">
                                    <q-btn flat @click="step = 3" unelevated icon="keyboard_arrow_up" :color="$store.state.settings.lightMode === 'false' ? 'primary' : 'white'" class="--back-btn" />
...
export default {
  ...
  async created () {
    ...
    this.currentAccount = this.tableData.find(w => w.chain === this.params.chainID && w.type === this.params.tokenID && (
      w.chain === 'eos' ? w.name.toLowerCase() === this.params.accountName.toLowerCase() : w.key === this.params.accountName))
  ...
  methods: {
    ...
    upgradeAccountName () {
      ...
      this.$router.push('/verto/dashboard/' + this.accountName.value)
    },
    checkPassword () {
