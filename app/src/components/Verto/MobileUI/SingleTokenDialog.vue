              ...
              <div>
                <q-card :dark="$store.state.settings.lightMode === 'true'" flat bordered :class="$store.state.settings.lightMode === 'true' ? 'q-pa-md text-white': 'q-pa-md text-black' " :style="$store.state.settings.lightMode === 'true' ? 'background-color: #04111F !important;': ''">
                    <div>
                        ...
                        <div v-else-if="tab != 'send' && tab != 'import' || (tab == 'send' && asset.chain == 'eos')" class="q-pa-md send_wrapper column justify-between full-height">
                            ...
                            <div class="column" v-if="!fromPreview ">
                              <q-input
                              ...
                              <template v-slot:hint>
                                <q-btn outline unelevated no-caps class="set-max q-mt-sm q-mb-md q-pl-sm q-pr-sm" label="Set max" @click="$emit('update:depositQuantity',  parseFloat(assetBalance))" dense />
                              </template>
                            ...
                            >
                              <q-btn outline icon-right="send" color="blue-4" :label="tab === 'send' ? (!fromPreview ? 'Preview' : 'Send') : tab" unelevated class="full-width send_btn" @click="isTxValid && triggerActionLocal()" />
                            </div>
                        </div>
                    </div>
                </q-card>
