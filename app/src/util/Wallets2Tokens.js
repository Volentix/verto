...
//       const vtxCoin = wallet.type === 'verto' ? 'vtx' : wallet.type
//       const coinSlug = coinsNames.data.find(coin => coin.symbol.toLowerCase() === vtxCoin.toLowerCase())
...
//                 const type = t.symbol.toLowerCase()
//                 const coinSlug = coinsNames.data.find(coin => coin.symbol.toLowerCase() === type.toLowerCase())
//                 let vespucciScore = 0
//                 if (coinSlug) {
//                   this.getCoinScore(coinSlug.slug).then(result => {
//                     vespucciScore = result.vespucciScore
...
//                   const coinSlug = coinsNames.data.find(coin => coin.symbol.toLowerCase() === 'eos')
...
//           }).then(res => {
//             const tokenSets = res.data.rebalancing_sets
...
//     const coinEOS = (await axios.get('https://cors-anywhere.herokuapp.com/https://api.newdex.io/v1/price?symbol=' + contract + '-' + coin + '-eos')).data.data.price
//     const coinUSD = coinEOS * this.eosUSD
...
//         let scoreVespucci = 0
//         for (const accuracy of mydata.rating) {
//           if (accuracy.accuracy !== 0) {
//             scoreVespucci = accuracy.accuracy
//           }
//         }
//         const marketCap = mydata.marketcap.current_marketcap_usd
//         currentAsset = {
