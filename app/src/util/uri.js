...
const transactionAbi = require('../../node_modules/eosjs/src/transaction.abi.json')
...
class URI {
  constructor () {
    ...
    log.info(forbiddenActions)
  }
  broadcastURI (tx, blockchain, callback = false) {
    //   1. Create return object
    // {
    //   dispatch () => void
    //   getState ()=> value of getState()
    // }

    // 2. Get the state (connection and broadcast info)

    // 3. If no blockchain specified, set dispatch in the return object error code and error type
  ...
  callbackURI (tx, blockchain, callback = false) {
  ...
  callbackURIWithProcessed (callback) {
  // return async (dispatch: () => void, getState) => {
  //   dispatch({
  ...
  checkRequest (data) {
    const errors = this.forbiddenActions.map((prevent) => {
  ...
  signIdentityRequest (prompt, blockchain, wallet) {
  // return (dispatch: () => void, getState) => {
  //   const {
  //     auths,
  //     connection,
  ...
  signURI (tx, blockchain, wallet, broadcast = false, callback = false) {
  // return (dispatch: () => void, getState) => {
  //   const {
  //     auths,
  //     connection,
