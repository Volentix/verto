      ...
      <q-stepper dark color="green bg-white" ref="stepper" alternative-labels :contractable="contractable">
        ...
        <q-step default name="first" title="Private Key" class=" bg-black workflow-step">
          <q-card-main class=" text-white"  >
            ...
            <div class="flex text-center q-title" v-for="part in privatekeyParts"  :key="part">
                <br><br>
                {{ part }}
            </div>
          </div>
                ...
                <span class="q-pa-sm">
                  <q-field  >
                    <q-input
                      ...
                      @input="checkPassword"
                      @keyup.enter="submit"
                      :float-label="$t('DisplayKey.verto_password')"
                    />
                  </q-field>
                </span>
                <div v-show="nameAlreadyUsed" class="q-title uppercase text-red">
                  {{ $t('DisplayKey.name_is_used') }}
                </div>
                <div v-show="passwordInvalid" class="q-title uppercase text-red">
  ...
  methods: {
    ...
    submit: async function () {
      ...
      if (result && result.success) {
        ...
        this.publicKey = ''
        this.privateKey = ''
        this.$q.notify({type: 'positive', message: 'Wallet created'})
